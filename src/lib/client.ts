 ...
 */
async function requestAbsolute(method: Method, absoluteUrl: string, opts: RequestOptions = {}): Promise<unknown> {
  let url = absoluteUrl;
...
/** Client scoped to Readwise API v2 (https://readwise.io/api/v2) */
export const clientV2 = {
  get(path: string, params?: Record<string, string>) {
    return requestAbsolute("GET", `https://readwise.io/api/v2${path}`, { params });
  },
  post(path: string, body?: Record<string, unknown>) {
    return requestAbsolute("POST", `https://readwise.io/api/v2${path}`, { body });
  },
  patch(path: string, body?: Record<string, unknown>) {
    return requestAbsolute("PATCH", `https://readwise.io/api/v2${path}`, { body });
  },
  delete(path: string) {
    return requestAbsolute("DELETE", `https://readwise.io/api/v2${path}`);
  },
};
