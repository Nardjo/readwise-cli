...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  readwise-cli auth set sk-abc123xyz")
  .action((token: string) => {
    setToken(token);
...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  readwise-cli auth show\n  readwise-cli auth show --raw")
  .action((opts: { raw?: boolean }) => {
    if (!hasToken()) {
      log.warn("No token configured. Run: readwise-cli auth set <token>");
      return;
    }
